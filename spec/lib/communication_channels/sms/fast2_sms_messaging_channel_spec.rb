...
RSpec.describe CommunicationChannels::Sms::Fast2SmsMessagingChannel do
  ...
  describe '#send_message' do
    before do
      @variables_values = { 'var1' => 'test value' }
      stub_fast2sms_api(user.phone, 'test value', template_id)
    end

    it 'able to send sms' do
      response = subject.send_message(user.phone, @variables_values, template_id)
      expect(response).to be_a(Net::HTTPOK)
    end

    it 'does not send sms if message id is invalid' do
      response = subject.send_message(user.phone, @variables_values, '')
