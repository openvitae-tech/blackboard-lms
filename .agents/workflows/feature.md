...
1. **Sync with main**
   Before any work, pull latest `main` into `feature/content-studio-v1`:
   ```bash
...
4. **UI Audit** *(skip if no Figma URL or no NeoComponent changes needed)*
...
6. **NeoComponent extensions** *(if needed)*
...
8. **Visual review loop** *(for UI changes)*
